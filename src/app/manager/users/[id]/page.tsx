...
import { useManagerUser, useManagerResumeUrl } from '@/hooks/useManager'
...
function ResumeDownloadButton({ resumeId, fileName }: { resumeId: string; fileName: string }) {
  const { refetch, isFetching } = useManagerResumeUrl(resumeId)
...
export default function ManagerUserProfilePage() {
  const { id } = useParams<{ id: string }>()
  const { data: user, isLoading } = useManagerUser(id)
