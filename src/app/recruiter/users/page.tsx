...
import { useRecruiterUsers, useRecruiterSkills } from '@/hooks/useRecruiter'
...
const PLANS = ['FORGE', 'LAUNCH', 'MOMENTUM']
...
export default function RecruiterUsersPage() {
  ...
  const { data, isLoading } = useRecruiterUsers({ page, limit: 20, search, skills: skillFilters, visaType: visaFilter, plan: planFilter })
  const { data: allSkills = [] } = useRecruiterSkills()
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-navy/5">
              {users.map((u) => (
                <tr
                  key={u.id}
                  onClick={() => router.push(`/recruiter/users/${u.id}`)}
